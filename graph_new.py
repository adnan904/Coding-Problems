...
def dfs(node, parent, counter, prev_max, graph, visited, stack):
    visited[node] = True
    stack.append(node)
    ...
    for child in graph[node]:
        ...
        if visited[child] is False:
            if dfs(child, node, counter, prev_max, graph=graph, visited=visited, stack=stack):
                return True
        else:
            if child in stack:
                stack.append(child)
...
if __name__ == '__main__':
    graph_string_input = "11:2,5;7:8,11;8:3,9,10"
    ...
    if len(roots):
        for node in roots:
            ...
            if dfs(node, -1, 0, 0, graph=graph, visited=visited, stack=stack):
                cyclic = True
                break
    else:
        visited = dict.fromkeys(nodes, False)
        sources = list(graph.keys())
        if dfs(sources[0], -1, 0, 0, graph=graph, visited=visited, stack=stack):
            cyclic = True
    if cyclic:
        stack = stack[::-1]
        index = -1
